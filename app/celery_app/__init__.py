...
@celery_app.task
def check_if_item_out_of_overstock_task(overstock_items_list):
    ...
    for user in users:
        ...
        for item in following_items:
            if item not in [elem["market_hash_name"] for elem in overstock_items_list]:
                logger.info(f"item {item} out of overstock for user {user['user_id']}")
                logger.info("check_if_item_out_of_overstock_task: FINISHED")
                task_id = send_notification_task.delay(user, item)
                set_task_id_in_user_state(redis_storage, user["chat_id"], task_id)
                return
...
def send_notification_task(user, item):
    ...
    while item in following_items:
        msg_text = f"Вещь {item} вышла из оверстока!!!! А это значит я расскажу тебе заебатый анекдотик\n\n"
        try:
